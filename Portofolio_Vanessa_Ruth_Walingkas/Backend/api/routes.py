...
def portfolio():
    try:
        ...
        current_app.logger.exception("Portfolio data request failed")
        return jsonify({"error": "Data portfolio sedang tidak tersedia."}), 503
...
def contact():
    ...
    try:
        ...
        current_app.logger.exception("Contact message persistence failed")
        return jsonify({"error": "Pesan tidak dapat disimpan saat ini."}), 503
